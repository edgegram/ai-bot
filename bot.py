...
# Короткие ASCII-коды для callback_data (у Telegram лимит 64 байта,
# кириллица не помещалась и ловила ошибку BUTTON_DATA_INVALID).
# Полные тексты хранятся отдельно и подставляются по коду.

GENDER_OPTIONS = {
    "guy": "парень",
    "girl": "девушка",
    "blogger": "энергичный блогер-вайб",
    "streamer": "спокойный стример-вайб",
}

PERSONALITY_OPTIONS = {
    "caring": "заботливый, поддерживающий, внимательный",
    "sassy": "дерзкий, ироничный, с подколками",
    "energetic": "энергичный, эмоциональный, много восклицаний",
    "calm": "спокойный, рассудительный, немногословный",
    "sarcastic": "саркастичный, остроумный, с чёрным юмором",
}
...
def gender_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=labels[code], callback_data=f"gender:{code}")]
        for code in GENDER_OPTIONS
    ])


def personality_keyboard() -> InlineKeyboardMarkup:
    labels = {
        "caring": "Заботливый(ая)",
        "sassy": "Дерзкий(ая)",
        "energetic": "Энергичный(ая)",
        "calm": "Спокойный(ая)",
        "sarcastic": "Саркастичный(ая)",
    }
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=labels[code], callback_data=f"personality:{code}")]
        for code in PERSONALITY_OPTIONS
    ])
...
async def set_gender(callback: CallbackQuery, state: FSMContext):
    ...
    gender = GENDER_OPTIONS.get(code, code)
...
async def set_personality(callback: CallbackQuery, state: FSMContext):
    code = callback.data.split(":", 1)[1]
    personality = PERSONALITY_OPTIONS.get(code, code)
